...
def population_from_recvbuf(recvbuf_dict, SolModel, config):
    ...
    recvbuf_genes = recvbuf_genes.reshape(
        (config["runtime"]["n_organisms"], config["runtime"]["genes_size"])
    )
    recvbuf_state = recvbuf_state.reshape(
        (config["runtime"]["n_organisms"], *config["runtime"]["state_shape"])
    )
    ...
    for i in range(config["runtime"]["n_organisms"]):
        ...
        sol = SolModel(recvbuf_genes[i].copy(), state=state.copy())
