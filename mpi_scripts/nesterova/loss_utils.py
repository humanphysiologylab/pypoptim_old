...
def calculate_loss_cat(cat_control, cat_model):
    cat_control, _ = align_rise(
        signal_model=cat_model.to_numpy(), signal_control=cat_control.to_numpy()
    )
...
def calculate_loss_AP_CaT_restcurves(sol, exp_cond_name, config):
    ...
    for name in exp_cond["type"]:
        if name == "CaT":
            ...
            cat_model = (
                sol["phenotype"][exp_cond_name]["Ca_i"][-n_samples_per_stim:] * 1000
            )  # mM -> uM
            ...
            loss_cat = 10 * np.abs(DCa_control - DCa_model) + np.abs(
                CTA_control - CTA_model
            )
