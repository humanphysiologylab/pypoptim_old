...
legend_constants = pd.DataFrame(
    [
        ...
        },
        {
            ...
            "units": "picoA_per_millimolar_4",
        },
        {
            "name": "d_NaCa",
            "component": "Na_Ca_ion_exchanger_current",
            "units": "per_millimolar_4",
        },
        {
            "name": "gamma_Na",
            "component": "Na_Ca_ion_exchanger_current",
        ...
        },
        {
            "name": "Fluo-3_koff",
            "component": "intracellular_Ca_buffering",
            "units": "per_second_per_millimolar",
        },
    ]
)
...
def compute_rates_algebraic(t, S, C, R=None, A=None):
    ...
    A[46] = (C[14] * (np.power(S[10] / 1.00000, 0.445700)) * (S[0] - A[43])) / (
        1.00000 + np.exp((1.50000 * ((S[0] - A[43]) + 3.60000) * C[2]) / (C[0] * C[1]))
    )
    ...
    A[54] = (
        ((((C[20] * S[10]) / (S[10] + C[19])) * A[53]) / (A[53] + C[21]))
        * (S[0] + 150.000)
    ) / (S[0] + 200.000)
    A[1] = np.floor(t / C[5]) * C[5]
    A[24] = C[7] if ((t - A[1] >= C[4]) and (t - A[1] <= C[4] + C[6])) else 0
    R[11] = -(
        ((A[44] + A[45] + A[46] + A[47] + A[49]) - 2.00000 * A[54]) + A[24] * C[3]
    ) / (C[29] * C[2])
    R[10] = (C[39] - S[10]) / C[35] + (
        (A[44] + A[45] + A[46] + A[47] + A[49]) - 2.00000 * A[54]
    ) / (C[33] * C[2])
    ...
    A[37] = (
        ...
        * (np.exp(((S[0] - A[35]) * C[2]) / (C[0] * C[1])) - 1.00000)
    ) / (np.exp((S[0] * C[2]) / (C[0] * C[1])) - 1.00000)
    A[50] = C[17] * (S[0] - A[35])
    A[56] = (
        C[24]
        * (
            S[2] * S[2] * S[2] * S[18] * np.exp((C[2] * S[0] * C[26]) / (C[0] * C[1]))
            - S[1]
            * S[1]
            * S[1]
            * S[19]
            * np.exp(((C[26] - 1.00000) * S[0] * C[2]) / (C[0] * C[1]))
        )
    ) / (1.00000 + C[25] * (S[1] * S[1] * S[1] * S[19] + S[2] * S[2] * S[2] * S[18]))
    ...
    E_Ca = ((C[0] * C[1]) / (2.0 * C[2])) * np.log(
        S[18] / S[6]
    )  # C->R * C->T / (2 * C->F) * log(S->Ca_c / S->Ca_d)
    A[41] = C[9] * S[7] * (A[39] * S[8] + (1.0 - A[39]) * S[9]) * (S[0] - E_Ca)

    A[51] = ((C[0] * C[1]) / (2.00000 * C[2])) * np.log(S[18] / S[19])
    A[52] = C[18] * (S[0] - A[51])
    A[55] = (C[22] * S[19]) / (S[19] + C[23])
    R[18] = (C[38] - S[18]) / C[36] + ((A[41] + A[52] + A[55]) - 2.00000 * A[56]) / (
        2.00000 * C[33] * C[2]
    )
    R[1] = (C[37] - S[1]) / C[34] + (
        A[37] + A[50] + 3.00000 * A[56] + 3.00000 * A[54] + C[28]
    ) / (C[33] * C[2])
    ...
    A[57] = (
        (
            ...
            / (
                1.00000
                + (9.13652 * (np.power(1.00000, 0.477811)))
                / (np.power(C[27], 0.477811))
            )
        )
        * (0.0517000 + 0.451600 / (1.00000 + np.exp((S[0] + 59.5300) / 17.1800)))
        * (S[0] - A[43])
        * C[3]
    )
    A[59] = (
    ...
    R[0] = -A[59] * 1000.00
    A[60] = 200000.0 * S[19] * (1.00000 - S[20]) - 476.000 * S[20]
    R[20] = A[60]
    A[61] = 78400.0 * S[19] * (1.00000 - S[21]) - 392.000 * S[21]
    R[21] = A[61]
    A[62] = 200000.0 * S[19] * ((1.00000 - S[22]) - S[23]) - 6.60000 * S[22]
    R[22] = A[62]
    A[63] = 0.0800000 * A[61] + 0.160000 * A[62] + 0.0450000 * A[60]
    R[24] = A[63]

    #  Fluo-3
    A[70] = C[52] * S[19] * (C[51] - S[30]) - C[53] * S[30]
    R[30] = A[70]
    # print(t, S[30], R[70])

    A[67] = (C[40] * (S[19] / C[41] - (C[43] * C[43] * S[26]) / C[42])) / (
        (S[19] + C[41]) / C[41] + (C[43] * (S[26] + C[42])) / C[42]
    )
    A[64] = S[29] / (S[29] + 0.250000)
    A[65] = A[64] * A[64]
    A[66] = C[44] * A[65] * (S[25] - S[19])
    R[19] = (
        -((A[52] + A[55] + A[67]) - (A[58] + A[66] + 2.00000 * A[56]))
        / (2.00000 * C[29] * C[2])
        - 1.00000 * A[63]
        - A[70]
    )
