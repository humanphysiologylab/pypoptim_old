...
int run_chain(double *S, double *C,
    ...
    for (int i = 0; i < chain_length; ++i) {
        ...
        if (i > 0) {
            data_chain[1 + i * (C_SIZE + A_SIZE) + 7] = 0; // stim_amplitude
        }
    }
    ...
    for (int j = 0; j < chain_length; ++j) {
        ...
        atol_chain[j * S_SIZE + 6]     *=  1e-5;  // Ca_d
        atol_chain[j * S_SIZE + 19]    *=  1e-4;  // Ca_i
    }
    /* CHAIN -- DONE */
    ...
    for (int i_beat = 0; i_beat < n_beats; ++i_beat) {
        ...
        for (; i_out_local <= n_samples; i_out_local++, i_out_global++) {
            ...
            data_chain[1 + 0 * (C_SIZE + A_SIZE) + 7] = (i_out_local == 1) ? C[7] : 0; // stim_amplitude

            t_out = i_out_global * t_sampling;

            if (FLAG_EULER_CHAIN) {
                euler(&t, S_chain, data_chain, 1e-5, t_out);
            } else {
